...
const CSV_PATH = path.join(import.meta.dirname, "..", "data", "ali_express_links.csv");
const UPLOAD_PATH = path.join(import.meta.dirname, "..", "data", "links-to-upload.csv");
const REVIEWS_DIR = path.join(import.meta.dirname, "..", "src", "reviews");
const BOOK_REVIEWS_DIR = path.join(import.meta.dirname, "..", "src", "book-reviews");
...
export function resolveAliExpress(url, title) {
  const map = loadMap();
...
function collectReviewFilesIn(dir) {
  ...
  for (const entry of fs.readdirSync(dir)) {
    ...
    for (const f of fs.readdirSync(sub)) {
      if (/\.md$/i.test(f)) out.push(path.join(sub, f));
    }
  }
  return out;
}
...
function collectReviewFiles() {
  return collectReviewFilesIn(REVIEWS_DIR).concat(collectReviewFilesIn(BOOK_REVIEWS_DIR));
}
...
export function updateUploadCsv() {
  updateCsv();
  const map = loadMap();
